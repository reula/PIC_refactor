...
run_name_full = "thermal_rel_newmodule_J$(J[1])x$(J[2])_N$(N_exp)_Th$(θ_exp)_alp$(alpha_exp)_o$(order)_test"
...
save_averages(out_file, 1, u, g, N)
...
let t = t_i
    ...
    for k in 2:M
        ...
        if (k - 1) % out_every == 0
            j = (k - 1) ÷ out_every + 1
            save_averages(out_file, j, u, g, N)
